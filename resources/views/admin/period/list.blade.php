                ...
                @foreach($data as $row)
                    ...
                    <td>
                        <form id="frm_{{$row->id}}"  action="{{ route('period.destroy', $row->id) }}" method="post">
                            @csrf
                            @method('DELETE')
                            <a href="{{ route('period.edit', $row->id) }}" style="color: #333333" ><button type="button" class="btn btn-success btn-sm"><span id="collapse-icon" class="fa fa-edit" ></span></button></a>
