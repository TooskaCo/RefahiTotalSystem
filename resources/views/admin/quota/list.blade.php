            ...
            <table class="table table-striped table-hover">
                ...
                @foreach($data as $row)
                    <tr>
                        ...
                        <td>{{ $row->ToDate }}</td>
                        <td>
                            <form id="frm_{{$row->id}}"  action="{{ route('quota.destroy', $row->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <a href="{{ route('quota.edit', $row->id) }}" style="color: #333333" ><button type="button" class="btn btn-success btn-sm"><span id="collapse-icon" class="fa fa-edit" ></span></button></a>
